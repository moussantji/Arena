/* Génère l'image UNIFIÉE : 3 palettes en une — gestion de clinique, photos en fond */
const fs = require('fs');
...
/* =====================================================================
   FEUILLE UNIFIÉE (2880×1560)
===================================================================== */
function sheet() {
  let s = `<defs>
  <radialGradient id="gl1" cx="0.18" cy="0.1" r="0.7"><stop offset="0" stop-color="${P[1].ga}" stop-opacity="0.14"/><stop offset="1" stop-color="${P[1].ga}" stop-opacity="0"/></radialGradient>
  ...
  <radialGradient id="gl3" cx="0.85" cy="0.15" r="0.7"><stop offset="0" stop-color="${P[3].ga}" stop-opacity="0.14"/><stop offset="1" stop-color="${P[3].ga}" stop-opacity="0"/></radialGradient>
  ...
  <filter id="shd" x="-30%" y="-30%" width="160%" height="160%"><feDropShadow dx="0" dy="24" stdDeviation="36" flood-color="#000000" flood-opacity="0.55"/></filter>
</defs>`;
  /* fond texture */
  s += `<image href="${IMG}/bg-texture-sombre.jpg" x="0" y="0" width="2880" height="1560" preserveAspectRatio="xMidYMid slice"/>`;
  s += `<rect width="2880" height="1560" fill="rgba(8,9,14,0.88)"/>`;
  s += `<rect width="2880" height="1560" fill="url(#gl1)"/><rect width="2880" height="1560" fill="url(#gl2)"/><rect width="2880" height="1560" fill="url(#gl3)"/>`;
  /* header */
  s += `<text x="80" y="90" font-family="Manrope" font-weight="800" font-size="12" fill="#9AA1B8" letter-spacing="3">OCULIS · GESTION DE CLINIQUE OPHTALMO — RÉCEPTION, PILOTAGE &amp; FACTURATION</text>`;
  s += `<text x="76" y="196" font-family="Fraunces" font-weight="600" font-size="72" fill="#F4F5FB">Les 3 palettes <tspan font-style="italic" font-weight="300">en un</tspan></text>`;
  s += `<text x="80" y="240" font-family="Manrope" font-weight="600" font-size="14.5" fill="#8F96B0">Écran de pilotage &amp; réception · PC (Electron) + app mobile (React Native) · photos de la clinique intégrées en fond</text>`;
  s += `<rect x="2428" y="66" width="372" height="66" rx="14" fill="rgba(255,255,255,0.045)" stroke="rgba(255,255,255,0.13)"/>`;
  s += I('electron', 2448, 86, 28, '#8FB3FF', 1.6);
  s += I('rn', 2486, 86, 28, '#37A37F', 1.6);
  s += `<text x="2530" y="96" font-family="Manrope" font-weight="800" font-size="15" fill="#F4F5FB" letter-spacing="1">PC ELECTRON + MOBILE RN</text>`;
  s += `<text x="2530" y="115" font-family="Manrope" font-weight="600" font-size="10.5" fill="#8F96B0">3 palettes · 1 design system · v2</text>`;

  /* colonnes */
  [1, 2, 3].forEach(k => {
    const p = P[k], x0 = 80 + (k - 1) * 920;
    /* mockup desktop */
    s += `<g filter="url(#shd)"><rect x="${x0}" y="280" width="880" height="720" rx="16" fill="#10131B"/></g>`;
    s += `<g transform="translate(${x0} 280)">${deskMock(p, k)}</g>`;
    /* nom + mood */
    s += `<text x="${x0}" y="1082" font-family="Fraunces" font-weight="600" font-size="42" fill="#F4F5FB">${p.num} — ${esc(p.nameA)} <tspan font-style="italic" font-weight="300">${esc(p.nameB)}</tspan></text>`;
    let cx = x0;
    p.mood.forEach(m => {
      const w = chipW(m.toUpperCase(), 9.5);
      s += `<rect x="${cx}" y="1102" width="${w}" height="26" rx="13" fill="none" stroke="rgba(255,255,255,0.16)"/>`;
      s += `<text x="${cx + w / 2}" y="1119" font-family="Manrope" font-weight="800" font-size="9.5" fill="#B9C0D8" text-anchor="middle" letter-spacing="1.4">${m.toUpperCase()}</text>`;
      cx += w + 10;
    });
    /* dégradé */
    s += `<rect x="${x0}" y="1148" width="690" height="38" rx="10" fill="url(#gr${k})"/>`;
    s += `<text x="${x0 + 14}" y="1172" font-family="JetBrains Mono" font-size="11" fill="#FFFFFF" font-weight="700">linear-gradient(135deg, ${p.ga}, ${p.gb})</text>`;
    /* tokens */
    const tk = [
    ...
    tk.forEach((t, i) => {
      const x = x0 + (i % 4) * 173, y = 1204 + Math.floor(i / 4) * 62;
      s += `<rect x="${x}" y="${y}" width="165" height="54" rx="11" fill="rgba(255,255,255,0.035)" stroke="rgba(255,255,255,0.08)"/>`;
      s += `<rect x="${x + 11}" y="${y + 11}" width="32" height="32" rx="8" fill="${t[1]}" stroke="rgba(255,255,255,0.12)"/>`;
      s += `<text x="${x + 53}" y="${y + 24}" font-family="Manrope" font-weight="700" font-size="10.5" fill="#E9EBF4">${esc(t[0])}</text>`;
      s += `<text x="${x + 53}" y="${y + 40}" font-family="JetBrains Mono" font-size="9.5" fill="#8F96B0">${t[1]}</text>`;
    });
    /* usage + specs */
    s += `<text x="${x0}" y="1352" font-family="Manrope" font-weight="600" font-size="12" fill="#B9C0D8">${esc(p.use)}</text>`;
    p.specs.forEach((sp, i) => {
      const x = x0 + i * 173;
      s += `<rect x="${x}" y="1372" width="160" height="30" rx="9" fill="rgba(255,255,255,0.04)" stroke="rgba(255,255,255,0.1)"/>`;
      s += `<text x="${x + 80}" y="1391" font-family="JetBrains Mono" font-size="9.5" fill="#B9C0D8" text-anchor="middle">${sp}</text>`;
    });
    /* téléphone */
    s += `<g transform="translate(${x0 + 723} 1096) scale(0.36)"><g filter="url(#shd)"><rect x="0" y="0" width="380" height="860" rx="52" fill="#151823"/></g>${phoneMock(p, 'u' + k)}</g>`;
  });

  /* footer */
  s += `<text x="80" y="1512" font-family="Manrope" font-weight="800" font-size="10.5" fill="#6E7690" letter-spacing="2">OCULIS · 3 PALETTES EN UN — GESTION DE CLINIQUE OPHTALMO</text>`;
  s += `<text x="1440" y="1512" font-family="Manrope" font-weight="700" font-size="10.5" fill="#6E7690" text-anchor="middle" letter-spacing="1.5">CONTRASTE AA · FOND : RECEPTION CLINIQUE, SALLE D’ATTENTE, IRIS, IMAGERIE</text>`;
  s += `<text x="2800" y="1512" font-family="JetBrains Mono" font-size="10.5" fill="#6E7690" text-anchor="end">export : index.html · electron/ + react-native/</text>`;
  return `<svg xmlns="http://www.w3.org/2000/svg" width="2880" height="1560" viewBox="0 0 2880 1560">${s}</svg>`;
}
...
const r = new Resvg(svg, { font: resvgFontOpts, fitTo: { mode: 'width', value: 2880 } });
const png = r.render().asPng();
fs.writeFileSync(path.join(OUT, '00-3-palettes-en-1-clinique.png'), png);
console.log('OK 00-3-palettes-en-1-clinique.png', (png.length / 1024).toFixed(0) + ' Ko');
